...
PLAYER_VELOCITY = 10
COIN_STARING_VELOCITY = 10
COIN_ACCELERATION = 0.5
BUFFER_DISTANCE = 100

score = 0
player_lives = PLAYER_STARTING_LIVES
coin_velocity = COIN_STARING_VELOCITY
...
def handle_collisions():
    ...
    if player_rect.colliderect(player_rect, coin_rect):
        score += 1
        coin_sound.play()
...
def game_over_check():
    global score, running, player_lives, coin_velocity
    if player_lives == 0:
        blit(display_surface, game_over_text, game_over_rect)
        blit(continue_text, continue_rect)
        #### continue
        update_display()
